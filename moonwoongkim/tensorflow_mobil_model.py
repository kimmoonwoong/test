...
def FSDDataset():          # wav데이터 변환
    ...
    for index, row in dataset.iterrows():
        ...
        mfcc = librosa.feature.mfcc(y=data, sr=22050, n_mfcc=40, n_fft=400)
        mfcc = slice(mfcc, 80)
        ...
        train_y.append(label_check)

    print("데이터 생성 완료")


def UrBanDataset():    # wav파일 변환
...
def AI_HubDataset():
    ...
    label_setting_aihub = {"차량경적": 1, "차량주행음": 5, "차량사이렌": 4, "이륜차경적": 1, "개": 2} # 새롭게 라벨링
    for s in Ai_Hub_type_path:
        ...
        for filename in filelist:
            ...
            with open(file_path, 'rt', encoding='UTF8') as file:
                ...
                label_check = []
                if class_label in label_list: # 우리가 쓸 데이터를 뽑아 냄
    ...
    for s in AI_HubAlert_type_path:
        ...
        for filename in fill_list:
            ...
            with open(file_path, 'rt', encoding='UTF8') as file:
                ...
                features = np.concatenate([mfcc, delta_mfcc, delta_mfcc2], axis=0)
                train_X.append(features)
                train_y.append(label_check)                  # 데이터 저장
    print("데이터 생성 완료")

def AI_HubAlertsoundArgumentDataset():
...
AI_HubDataset()
AI_HubAlertDataset()
AI_HubAlertsoundArgumentDataset()
