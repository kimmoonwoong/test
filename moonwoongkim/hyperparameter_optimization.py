...
def AI_HubAlertDataset(train_set):
    ...
    label_setting_ai_hubAleart = {"도난경보 소리": 8, "도난 경보음 소리": 8, "침입감지 경보 소리": 8, "화재경보 소리": 7, "화재 경보 소리": 7,
                                  "가스누설 화재경보 소리": 7, "자동차 경적 소리": 1, "비상경보 소리": 6, "철도 건널목 신호음 소리": 6,
                                  "민방위훈련 사이렌 소리": 6, "공습경보 소리": 6}  # 새롭게 라벨링
...
def extract_feature(data, label, isCheck):  # 위에서 변환한 데이터를 mfcc의 이용으로 소리 데이터를 벡터화 시킴( 벡터화 시킴으로써 CNN모델을 사용할 수 있게 됨 )
    mfccs = []
    slice = lambda a, i: a[:, 0:i] if a.shape[1] > i else np.hstack(
        (a, np.zeros((a.shape[0], i - a.shape[1]))))  # 데이터의 길이를 설정한 길이에 맞게 맞춰줌
...
class CNNclassification(torch.nn.Module):  # 4중 layer로 구현
    def __init__(self, kernel_size=2, stride=2, padding=1):
        ...
        self.layer1 = torch.nn.Sequential(
            nn.Conv2d(40, 100, kernel_size=kernel_size, stride=stride, padding=padding),  # cnn layer
            nn.ReLU(),  # activation function
            nn.MaxPool2d(kernel_size=kernel_size, stride=stride))  # pooling layer

        self.layer2 = torch.nn.Sequential(
            nn.Conv2d(100, 100, kernel_size=kernel_size, stride=stride, padding=padding),  # cnn layer
            nn.ReLU(),  # activation function
            nn.MaxPool2d(kernel_size=kernel_size, stride=stride))  # pooling layer
...
def prediction(model, predic_data, device):
    ...
    with torch.no_grad():
        for wav in tqdm(iter(predic_data)):
            ...
            predic_list.extend(pred.tolist())
    return predic_list
...
def hyperparameter_optimization():
    ...
    for i in range(0,100):
        kenel_size = random.randrange(1,5)
        stride = random.randrange(1,5)
        padding = random.randrange(1,5)
        size = 160
        check = False

        for j in range(0,4):
            size = (size - kenel_size + 2 * padding)/stride
            if size % 2 != 0:
                check = True
                break
            size = size / kenel_size
        ...
        model = CNNclassification(kenel_size, stride, padding).to(device)
        acc = train(model, optimizer, train_loader, scheduler, device)
        if best_acc < acc:
            best_acc = acc
            optimization = [kenel_size, stride, padding]
